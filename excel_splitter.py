...
Description:
    ...
    manageable chunks while preserving all cell formatting, styles, and formulas.
    The application runs the splitting process in a separate, non-blocking thread
    to ensure the user interface remains responsive, providing real-time progress
    updates and a cancellation option. This version includes a high-performance,
    style-caching copy method for significantly faster processing of large files.
...
The program executes the following numbered steps:
  ...
  3. Prompts the user to choose between a Standard (slower) or Optimized (faster)
     cell copying method.
  4. Prompts for splitting configuration: number of data rows per chunk and number
     of header rows to repeat in each new file.
  5. Presents a choice to preserve formulas or to save only their static,
     calculated values.
  6. Performs a pre-scan of the input file to accurately calculate the total number
     of chunks for the progress bar.
  7. Launches the main splitting operation in a separate, non-blocking thread.
  8. Displays a progress window showing the current status and a progress bar,
     complete with a "Cancel" button.
  9. The worker thread iterates through each chunk, using the user-selected copy
     method to preserve all cell formatting, styles, comments, and merged cells.
 10. Checks for the cancellation signal before processing each new chunk to allow
     for a graceful exit.
 11. Saves each completed chunk as a new `.xlsx` file in the chosen directory.
 12. Upon completion, error, or cancellation, displays a final summary report.
